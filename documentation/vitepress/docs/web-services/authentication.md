...
The Web Service validates this token by fetching Keycloak's JSON Web Key Set (JWKS) at `{keycloak.baseUrl}realms/{keycloak.realm}/protocol/openid-connect/certs` and verifying the token's signature, issuer and expiry — it never contacts Keycloak's token end-point itself. On top of that, it checks the token's `azp` (authorized-party) claim against the list of client IDs in `dlb.auth.keycloak.trusted-clients` (environment variable `DLB_AUTH_KEYCLOAK_TRUSTED_CLIENTS`): being signed by a trusted realm is not enough, the token must also have been issued to a client this deployment trusts. Left unset, the list defaults to the deployment's own `client-id` plus the BFF's client id (`dlb-bff`) — the BFF forwards tokens issued to its own client, and Dialogue Branch Studio reaches the Web Service only through the BFF, so this default covers the standard setup with no configuration. In a shared Keycloak realm where further back-end services authenticate against the same realm, set the property to a comma-separated list of their client IDs, or to a single `*` to accept a token from any client in a trusted realm. An explicit list replaces the default entirely, so a BFF-fronted deployment that sets it must still include `dlb-bff`. A token from an untrusted client is rejected with `401`, and the first rejection of each distinct `azp` is logged at `WARN`. The Swagger UI bundled with the Web Service is itself configured as an OAuth2 client (with PKCE) against the same Keycloak realm, so you can authenticate directly from the "Authorize" button on the Swagger page without writing any client code — see [Tutorial: Web Service - Exploring the API](/tutorials/webservice-exploringapi).
...
sequenceDiagram
    ...
    Keycloak-->>Browser: Redirect back to Studio
